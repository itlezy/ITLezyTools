...
namespace EPocalipse.Json.Viewer
{
    public partial class JsonViewer : UserControl
    {
        ...
        public JsonViewer()
        {
            ...
            tvJson.Font = new Font(tvJson.Font.Name, decimal.ToInt32(numFontSize.Value));
        }
        ...
        private void Redraw()
        {
            try
            {
                ...
                try
                {
                    ...
                    if (ckSortNodes.Checked)
                    {
                        tvJson.TreeViewNodeSorter = new NodeSorter(true);
                    }
                    else
                    {
                        tvJson.TreeViewNodeSorter = new NodeSorter(false);
                    }

                    if (ckAutoExpand.Checked)
                    {
                        tvJson.ExpandAll();
                    }

                    EnsureTopNodeIsVisible();
                }
                finally
                {
                    tvJson.EndUpdate();
        ...
        {
            txtJson.Text = ClipboardHelper.GetText();
        }
        ...
        private void mnuCopyName_Click(object sender, EventArgs e)
        {
            ...
            if (node != null && node.JsonObject.Id != null)
            {
                ...
                ClipboardHelper.SetText(obj.Id);
            }
            else
            {
        ...
        private void mnuCopyValue_Click(object sender, EventArgs e)
        {
            ...
            {
                JsonObject obj = node.Tag as JsonObject;
                ClipboardHelper.SetText(obj.Value.ToString());
            }
            else
            {
        ...
        private void ckSortNodes_CheckedChanged(object sender, EventArgs e)
        {
            Redraw();
        }
        ...
        private void EnsureTopNodeIsVisible()
        {
            if (tvJson.Nodes.Count > 0)
            {
        ...
        private void ckAutoExpand_CheckedChanged(object sender, EventArgs e)
        {
            tvJson.BeginUpdate();
    ...
    public class NodeSorter : IComparer
    {
        ...
        public NodeSorter(bool applySort)
        {
        ...
        public int Compare(Object x, Object y)
        {
            ...
            // always sort array elements [0], [1], ..
            if (!String.IsNullOrWhiteSpace(tx.Text) &&
                !String.IsNullOrWhiteSpace(ty.Text) &&
                Regex.IsMatch(tx.Text, "\\[\\d+\\]") &&
                Regex.IsMatch(ty.Text, "\\[\\d+\\]"))
            {
                int idx1 = int.Parse(SubstringBySeparators(tx.Text, "[", "]"));
                int idx2 = int.Parse(SubstringBySeparators(ty.Text, "[", "]"));

                return idx1.CompareTo(idx2);
            }

            return applySort ? String.Compare(tx.Text, ty.Text, true) : 0;
        }

        private String SubstringBySeparators(String str, String beginSep, String endSep)
        {
            return str.Substring(
                (str.IndexOf(beginSep) + beginSep.Length),
                (str.IndexOf(endSep) - str.IndexOf(beginSep) - beginSep.Length));
        }
    }
